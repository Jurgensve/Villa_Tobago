            ...
            elseif ($agent_final):
                ...
                if ($is_moving_out):
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                        ...
                        <!-- Intercom Card -->
                        <div class="bg-white shadow rounded-xl overflow-hidden border-t-4 border-blue-400 flex flex-col">
                            <div class="px-5 py-4 bg-gray-50 border-b flex justify-between items-center">
                                ...
                                <a href="intercom_form.php" class="text-xs text-blue-600 hover:underline font-bold">Edit</a>
                            </div>
                            <div class="p-5 flex-1 flex flex-col gap-3">
                                <?php if (isset($step_data['intercom_update_status']) && $step_data['intercom_update_status'] === 'Pending'): ?>
                                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                                        <h4 class="text-xs font-bold text-yellow-800 mb-1"><i class="fas fa-exclamation-circle"></i>
                                            Pending Update</h4>
                                        <p class="text-[11px] text-yellow-700 mb-2">Awaiting admin approval for:</p>
                                        <div class="space-y-1 mb-1">
                                            <?php if ($step_data['pending_ic1_phone']): ?>
                                                <p class="text-xs"><strong>1:</strong> <?= h($step_data['pending_ic1_name']) ?>
                                                    (<?= h($step_data['pending_ic1_phone']) ?>)</p>
                                            <?php endif; ?>
                                            <?php if ($step_data['pending_ic2_phone']): ?>
                                                <p class="text-xs"><strong>2:</strong> <?= h($step_data['pending_ic2_name']) ?>
                                                    (<?= h($step_data['pending_ic2_phone']) ?>)</p>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                <?php endif; ?>
                        ...
                        <!-- Vehicles Card -->
                        <div class="bg-white shadow rounded-xl overflow-hidden border-t-4 border-indigo-400 flex flex-col">
                            <div class="px-5 py-4 bg-gray-50 border-b flex justify-between items-center">
                                <h3 class="text-lg font-bold text-gray-800 flex items-center gap-2"><i
                                        class="fas fa-car text-indigo-400"></i> Vehicles</h3>
                                <div class="flex items-center gap-3">
                                    <span class="text-xs text-gray-500"><?= count($vehicles) ?> registered</span>
                                    <a href="manage_vehicles.php"
                                        class="text-xs text-indigo-600 hover:underline font-bold">Manage</a>
                                </div>
                            </div>
                            <div class="p-5 flex-1 flex flex-col gap-3">
                                <?php if (!empty($vehicles)): ?>
                                    <?php foreach ($vehicles as $v): ?>
                                        <div class="p-3 bg-indigo-50 rounded-lg">
                                            <div class="font-bold text-indigo-900 text-sm"><?= h($v['registration']) ?></div>
                                            <div class="text-indigo-700 text-xs"><?= h($v['make_model']) ?> &middot; <?= h($v['color']) ?>
                                            </div>
                                        </div>
                                    <?php endforeach; ?>
                                <?php else: ?>
                        ...
                        <!-- Pets Card -->
                        <div class="bg-white shadow rounded-xl overflow-hidden border-t-4 border-yellow-400 flex flex-col">
                            <div class="px-5 py-4 bg-gray-50 border-b flex justify-between items-center">
                                <h3 class="text-lg font-bold text-gray-800 flex items-center gap-2"><i
                                        class="fas fa-paw text-yellow-500"></i> Pets</h3>
                                <div class="flex items-center gap-3">
                                    <span class="text-xs text-gray-500">Max: <?= $max_pets == 0 ? 'Unlimited' : $max_pets ?></span>
                                    <a href="manage_pets.php" class="text-xs text-yellow-600 hover:underline font-bold">Manage</a>
                                </div>
                            </div>
                            <div class="p-5 flex-1 flex flex-col gap-3">
                                <?php if (!empty($pets)): ?>
                                        ...
                                        <div class="p-3 bg-yellow-50 rounded-lg flex items-start justify-between">
                                            <div>
                                                <div class="font-bold text-yellow-900 text-sm flex items-center gap-1">
                                                    <?= h($pet['name']) ?>
                                                    <span class="text-[10px] <?= $sc ?> px-1.5 py-0.5 rounded-full ml-1 whitespace-nowrap">
                                                        <?= h($pet['status'] ?? 'Pending') ?>
                                                    </span>
                                                </div>
                                                <div class="text-[11px] text-yellow-700 mt-1">
                                                    <?= h($pet['type']) ?>
                                    ...
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <p class="text-gray-400 italic text-sm">No pets registered.</p>
                                <?php endif; ?>
                            </div>
                        </div>
                    ...
                    <div class="bg-white rounded-2xl shadow p-6 flex flex-col gap-4 border-l-4 border-green-500 mb-6">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-gray-500 text-sm mb-3">Join the official Villa Tobago community group for residents
                                    and owners.</p>
