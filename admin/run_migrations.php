...
$migrations = [
    ...
    'residency' => [
        'label' => 'Residency Workflow Schema (residency_migration.sql)',
        'description' => 'Updates owner/tenant tables with portal access fields, creates vehicles table, adds pet compliance tracking.',
        'file' => __DIR__ . '/../residency_migration.sql',
        'icon' => 'fa-key',
        'color' => 'blue',
    ],
    'patch_gwm' => [
...
<div class="mb-6">
    ...
    <p class="text-gray-500 text-sm mt-1">Run schema migration scripts directly from the admin panel. Safe to re-run —
        all scripts use <code>IF NOT EXISTS</code> and <code>INSERT IGNORE</code>.</p>
</div>
...
<div class="mb-8 bg-white shadow rounded-lg overflow-hidden">
    <div class="px-6 py-4 border-b flex items-center gap-3
        ...
        <i
            class="fas <?= $err_count === 0 ? 'fa-check-circle text-green-500' : 'fa-exclamation-triangle text-red-500'?> text-xl"></i>
        <div>
            <h2 class="font-bold text-gray-800">
                <?= h($migration_label)?> — Run Complete
            </h2>
            <p class="text-sm mt-0.5">
                <span class="text-green-700 font-bold">
                    <?= $ok_count?> statement(s) OK
                </span>
                <?php if ($err_count > 0): ?>
    ...
    <div class="p-4 space-y-2 max-h-96 overflow-y-auto">
        ...
        <div
            class="rounded border <?= $r['ok'] ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'?> px-3 py-2 text-xs font-mono">
            <div class="flex items-start gap-2">
                <i
                    class="fas <?= $r['ok'] ? 'fa-check text-green-600' : 'fa-times text-red-600'?> mt-0.5 flex-shrink-0"></i>
                <pre
                    class="whitespace-pre-wrap break-all text-gray-700 flex-1"><?= h(mb_strimwidth($r['sql'], 0, 300, '…'))?></pre>
            </div>
            <?php if (!$r['ok'] && $r['error']): ?>
            <p class="mt-1 ml-5 text-red-700 font-sans font-bold">
                <?= h($r['error'])?>
            </p>
            <?php
        endif; ?>
        ...
        <div class="p-6">
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <?php if (!$file_exists): ?>
                    <p class="text-xs text-red-500 mt-2 font-bold"><i class="fas fa-exclamation-triangle mr-1"></i>SQL
                        file not found on server</p>
                    <?php
    else: ?>
...
<div class="mt-8 bg-gray-50 border border-gray-200 rounded-lg p-6">
    <h2 class="font-bold text-gray-700 mb-1"><i class="fas fa-layer-group mr-2 text-gray-400"></i>Run All Migrations in
        Order</h2>
    <p class="text-sm text-gray-500 mb-4">Executes all four scripts sequentially. Recommended for a fresh database
        setup. Safe to re-run on an existing database.</p>
    <form method="POST" id="runAllForm">
        <input type="hidden" name="run_all" value="1">
        <button type="button" onclick="runAllSequentially()"
