...
<div class="max-w-4xl mx-auto py-8 px-4">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <p class="text-gray-500 mt-2">Unit <?= h($res['unit_number']) ?> &middot; Register vehicles kept on the
                property permanently.</p>
        </div>
        <a href="resident_portal.php"
            class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-2.5 px-5 rounded-xl transition shadow-sm flex items-center gap-2">
    ...
    <?php if ($message): ?>
        <div
            class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-xl mb-6 text-sm flex items-center gap-2 shadow-sm">
    ...
    <?php if ($error): ?>
        <div
            class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-xl mb-6 text-sm flex items-center gap-2 shadow-sm">
                ...
                <?php else: ?>
                    <div class="space-y-4">
                        <?php foreach ($vehicles as $v): ?>
                            <div
                                class="border-2 border-gray-100 rounded-xl p-4 flex justify-between items-center bg-gray-50 hover:bg-white transition relative group">
                                <div>
                                    ...
                                    <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-sm text-gray-600">
                                        <div><span
                                                class="text-gray-400 text-xs">Make/Model:</span><br><?= h($v['make_model']) ?>
                                        </div>
                                ...
                                <form method="POST" onsubmit="return confirm('Are you sure you want to remove this vehicle?');">
                                    ...
                                    <button type="submit" name="delete_vehicle"
                                        class="w-10 h-10 rounded-full bg-red-50 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition shadow-sm">
            ...
            <div class="p-5">
                <?php if ($max_vehicles > 0 && count($vehicles) >= $max_vehicles): ?>
                    <div class="bg-yellow-50 border-2 border-yellow-200 rounded-xl p-6 text-center">
                        ...
                        <p class="text-sm text-yellow-700 mt-1">You have reached the maximum allowed vehicles
                            (<?= $max_vehicles ?>) for this unit.</p>
                    </div>
                <?php else: ?>
                    <form method="POST" class="space-y-4">
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-1">Registration (Number Plate) <span
                        ...
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-1">Make & Model <span
                                    class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-1">Color <span
                                    class="text-red-500">*</span></label>
                        ...
                        <button type="submit" name="add_vehicle"
                            class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-xl shadow-md transition flex justify-center items-center gap-2 mt-4">
